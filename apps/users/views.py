...
from .models import Address, UserPreferences
...
class RegisterView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        """处理注册请求，并发场景下自动重试避免 IntegrityError"""
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        try:
            user = serializer.save()
        except IntegrityError:
            # 并发场景：同用户名在前一步校验通过后被另一个请求抢占了
            # 追加毫秒后缀后重试一次
            import time as _time

            data = request.data.copy()
            data["username"] = f"{data['username']}_{int(_time.time() * 1000) % 10000}"
...
class AddressListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"code": 200, "message": "添加成功", "data": serializer.data},
            status=status.HTTP_201_CREATED,
        )
...
class SetDefaultAddressView(APIView):
    ...
    def put(self, request, pk):
        ...
        try:
            ...
            return Response(
                {"code": 404, "message": "地址不存在"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        return Response({"code": 200, "message": "设置成功"})
...
class UpgradeToSellerView(APIView):
    ...
    def post(self, request):
        ...
        if user.role == "seller":
            return Response(
                {"code": 400, "message": "您已经是商家，无需重复升级"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        contact_phone = request.data.get("contact_phone", "")  # noqa: F841

        # 验证店铺名称
        if not shop_name:
            return Response(
                {"code": 400, "message": "请输入店铺名称"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {
                "code": 200,
                "message": "升级成功",
                "data": {"role": user.role, "shop_name": user.shop_name},
            }
        )
...
class ShopSettingsView(APIView):
    ...
    def put(self, request):
        ...
        user.shop_description = request.data.get(
            "shop_description", user.shop_description
        )
        user.save()
...
class BindPhoneView(APIView):
    ...
    def post(self, request):
        ...
        if not phone or len(phone) != 11:
            return Response(
                {"code": 400, "message": "请输入正确的11位手机号"}, status=400
            )
        request.user.phone = phone
        request.user.save()
        return Response({"code": 200, "message": "绑定成功", "data": {"phone": phone}})
