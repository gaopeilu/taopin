...
class SearchHistoryListView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        data = [
            {"id": h.id, "keyword": h.keyword, "created_at": h.created_at}
            for h in history
        ]
...
class SearchSuggestView(APIView):
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            try:
                SearchHistory.objects.update_or_create(
                    user=request.user, keyword=q, defaults={}
                )
            except Exception:
                logger.warning(
                    "update_or_create failed for user=%s keyword=%s",
                    request.user.id,
                    q,
                    exc_info=True,
                )
        ...
        suggestions = list(
            GoodsSPU.objects.filter(
                name__icontains=q, is_deleted=False, is_on_sale=True
            )
