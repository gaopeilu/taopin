...
def _get_brand_id():
    if "brand_id" not in _SHARED:
        resp = requests.get(f"{BASE_URL}/goods/brands/")
...
class TestCategoryRead:
    ...
    def test_children_have_correct_level(self):
        """子分类 level >= 父级 level（不严格+1，因为后端不自动算）"""
        resp = requests.get(f"{BASE_URL}/goods/categories/")
        results = resp.json()["results"]
    ...
    def test_detail_by_id(self):
        """查单个分类详情 — 直接返回对象"""
        cat_id = requests.get(f"{BASE_URL}/goods/categories/").json()["results"][0]["id"]
        resp = requests.get(f"{BASE_URL}/goods/categories/{cat_id}/")
        assert resp.status_code == 200
        assert resp.json()["id"] == cat_id

    def test_detail_not_found(self):
        """不存在的分类 → 404"""
        resp = requests.get(f"{BASE_URL}/goods/categories/99999/")
        assert resp.status_code == 404

    def test_tree_endpoint(self):
        """tree/ 直接返回列表"""
        resp = requests.get(f"{BASE_URL}/goods/categories/tree/")
        assert resp.status_code == 200
        data = resp.json()
...
class TestCategoryWrite:
    ...
    def test_create_top_level(self):
        ...
        resp = requests.post(f"{BASE_URL}/goods/categories/", json={
            "name": name,
    ...
    def test_create_sub_category(self):
        """创建子分类 — level 自动+1"""
        parent_id = requests.get(f"{BASE_URL}/goods/categories/").json()["results"][0]["id"]
        ...
        assert data["level"] >= 1  # 服务端不自动算 level，只校验 >=1

    def test_create_no_name(self):
        """缺 name → 400"""
        resp = requests.post(f"{BASE_URL}/goods/categories/", json={}, headers={
    ...
    def test_create_no_token(self):
        """无 token → 401"""
        resp = requests.post(f"{BASE_URL}/goods/categories/", json={"name": "x"})
    ...
    def test_update_category(self):
        """修改分类名称"""
        create_resp = requests.post(f"{BASE_URL}/goods/categories/", json={
            "name": f"原始_{int(time.time() * 1000)}",
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        cat_id = create_resp.json()["id"]

        new_name = f"新名称_{int(time.time() * 1000)}"
        resp = requests.put(f"{BASE_URL}/goods/categories/{cat_id}/", json={
            "name": new_name,
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        assert resp.status_code == 200
    ...
    def test_delete_category(self):
        """删除分类 → 204 + 确认已删"""
        create_resp = requests.post(f"{BASE_URL}/goods/categories/", json={
            "name": f"待删_{int(time.time() * 1000)}",
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        cat_id = create_resp.json()["id"]

        resp = requests.delete(f"{BASE_URL}/goods/categories/{cat_id}/", headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
...
class TestBrandRead:
    ...
    def test_filter_by_letter(self):
        """按首字母筛选 — ?letter=A"""
        resp = requests.get(f"{BASE_URL}/goods/brands/?letter=A")
        assert resp.status_code == 200
    ...
    def test_filter_letter_empty_result(self):
        """不存在的字母 — 返回空"""
        resp = requests.get(f"{BASE_URL}/goods/brands/?letter=Z")
        assert resp.status_code == 200
        assert resp.json()["count"] == 0

    def test_detail(self):
        """品牌详情"""
        brands = requests.get(f"{BASE_URL}/goods/brands/").json()["results"]
        if brands:
            brand_id = brands[0]["id"]
            resp = requests.get(f"{BASE_URL}/goods/brands/{brand_id}/")
            assert resp.status_code == 200
            assert resp.json()["id"] == brand_id

    def test_detail_not_found(self):
        """不存在的品牌 → 404"""
        resp = requests.get(f"{BASE_URL}/goods/brands/99999/")
        assert resp.status_code == 404
...
class TestBrandWrite:
    ...
    def test_create(self):
        ...
        resp = requests.post(f"{BASE_URL}/goods/brands/", json={
            "name": name,
            "first_letter": "C",
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        assert resp.status_code == 201
    ...
    def test_create_no_name(self):
        """缺 name → 400"""
        resp = requests.post(f"{BASE_URL}/goods/brands/", json={}, headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
        assert resp.status_code == 400
    ...
    def test_create_no_token(self):
        """无 token → 401"""
        resp = requests.post(f"{BASE_URL}/goods/brands/", json={"name": "x"})
        assert resp.status_code == 401
    ...
    def test_update_brand(self):
        ...
        brand_id = requests.post(f"{BASE_URL}/goods/brands/", json={
            "name": name,
        }, headers={"Authorization": f"Bearer {self.seller_token}"}).json()["id"]

        resp = requests.put(f"{BASE_URL}/goods/brands/{brand_id}/", json={
            "name": f"改后_{int(time.time() * 1000)}",
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        assert resp.status_code == 200
    ...
    def test_delete_brand(self):
        ...
        resp = requests.delete(f"{BASE_URL}/goods/brands/{brand_id}/", headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
        assert resp.status_code == 204
...
class TestSPURead:
    ...
    def test_filter_on_sale(self):
        ...
        resp = requests.get(f"{BASE_URL}/goods/spus/?is_on_sale=true")
        assert resp.status_code == 200
    ...
    def test_filter_off_sale(self):
        ...
        resp = requests.get(f"{BASE_URL}/goods/spus/?is_on_sale=false")
        assert resp.status_code == 200
        for spu in resp.json()["results"]:
    ...
    def test_detail(self):
        ...
        spus = requests.get(f"{BASE_URL}/goods/spus/").json()["results"]
        if not spus:
            pytest.skip("没有 SPU 数据")
        spu_id = spus[0]["id"]
        resp = requests.get(f"{BASE_URL}/goods/spus/{spu_id}/")
        assert resp.status_code == 200
        detail = resp.json()
    ...
    def test_detail_not_found(self):
        """不存在的商品 → 404"""
        resp = requests.get(f"{BASE_URL}/goods/spus/99999/")
        assert resp.status_code == 404

    def test_hot(self):
        """热销商品"""
        resp = requests.get(f"{BASE_URL}/goods/spus/hot/")
        assert resp.status_code == 200
        data = resp.json()["data"]
        assert isinstance(data, list)
    ...
    def test_hot_with_limit(self):
        """热销商品 — ?limit=3"""
        resp = requests.get(f"{BASE_URL}/goods/spus/hot/?limit=3")
        assert resp.status_code == 200
        assert len(resp.json()["data"]) <= 3

    def test_search(self):
        """搜索商品"""
        resp = requests.get(f"{BASE_URL}/goods/spus/search/?q=手机")
        assert resp.status_code == 200
        assert "results" in resp.json()

    def test_search_empty_keyword(self):
        """搜索无关键词 → 服务端返回 400 或分页空结果"""
        resp = requests.get(f"{BASE_URL}/goods/spus/search/")
    ...
    def test_spu_skus(self):
        ...
        if not spus:
            pytest.skip("没有 SPU 数据")
        resp = requests.get(f"{BASE_URL}/goods/spus/{spus[0]['id']}/skus/")
        assert resp.status_code == 200
    ...
    def test_spu_images(self):
        ...
        if not spus:
            pytest.skip("没有 SPU 数据")
        resp = requests.get(f"{BASE_URL}/goods/spus/{spus[0]['id']}/images/")
        assert resp.status_code == 200
...
class TestSPUWrite:
    ...
    def test_create(self):
        ...
        resp = requests.post(f"{BASE_URL}/goods/spus/", json=body, headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
        assert resp.status_code == 201
        assert resp.json()["name"] == name
    ...
    def test_create_no_token(self):
        resp = requests.post(f"{BASE_URL}/goods/spus/", json={"name": "x"})
        assert resp.status_code == 401
    ...
    def test_toggle_sale(self):
        ...
        resp = requests.put(f"{BASE_URL}/goods/spus/{spu_id}/toggle_sale/", headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
        assert resp.status_code == 200
    ...
    def test_delete(self):
        """软删除商品"""
        spu_id = _create_spu_and_get_id(self.seller_token)
        resp = requests.delete(f"{BASE_URL}/goods/spus/{spu_id}/", headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
        assert resp.status_code == 204
        resp = requests.get(f"{BASE_URL}/goods/spus/{spu_id}/")
...
class TestSKURead:
    ...
    def test_filter_by_spu(self):
        ...
        if not spus:
            pytest.skip("没有 SPU 数据")
        resp = requests.get(f"{BASE_URL}/goods/skus/?spu={spus[0]['id']}")
        assert resp.status_code == 200

    def test_filter_in_stock(self):
        """筛选有库存 — ?in_stock=true"""
        resp = requests.get(f"{BASE_URL}/goods/skus/?in_stock=true")
        assert resp.status_code == 200
    ...
    def test_detail(self):
        """SKU 详情"""
        skus = requests.get(f"{BASE_URL}/goods/skus/").json()["results"]
        if not skus:
            pytest.skip("没有 SKU 数据")
        resp = requests.get(f"{BASE_URL}/goods/skus/{skus[0]['id']}/")
        assert resp.status_code == 200
...
class TestSKUWrite:
    ...
    def test_create(self):
        ...
        resp = requests.post(f"{BASE_URL}/goods/skus/", json={
            "spu": self.spu_id,
            "name": name,
            "price": "99.00",
            "stock": 100,
        ...
        assert resp.status_code == 201
        assert resp.json()["name"] == name
        assert resp.json()["is_in_stock"] is True

    def test_create_no_token(self):
        resp = requests.post(f"{BASE_URL}/goods/skus/", json={"name": "x"})
        assert resp.status_code == 401
    ...
    def test_deduct_stock(self):
        """扣库存"""
        sku_id = requests.post(f"{BASE_URL}/goods/skus/", json={
            "spu": self.spu_id, "name": f"扣库存SKU_{int(time.time() * 1000)}",
            "price": "50.00", "stock": 10,
        }, headers={"Authorization": f"Bearer {self.seller_token}"}).json()["id"]

        resp = requests.post(f"{BASE_URL}/goods/skus/{sku_id}/deduct_stock/", json={
            "quantity": 3
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        assert resp.status_code == 200
    ...
    def test_deduct_insufficient(self):
        """超库存扣减 → 400"""
        sku_id = requests.post(f"{BASE_URL}/goods/skus/", json={
            "spu": self.spu_id, "name": f"超库存_{int(time.time() * 1000)}",
            "price": "10.00", "stock": 2,
        }, headers={"Authorization": f"Bearer {self.seller_token}"}).json()["id"]
    ...
    def test_deduct_negative_quantity(self):
        """负数扣库存 → 400"""
        sku_id = requests.post(f"{BASE_URL}/goods/skus/", json={
            "spu": self.spu_id, "name": f"负数_{int(time.time() * 1000)}",
    ...
    def test_restore_stock(self):
        """恢复库存"""
        sku_id = requests.post(f"{BASE_URL}/goods/skus/", json={
            "spu": self.spu_id, "name": f"恢复库存_{int(time.time() * 1000)}",
            "price": "30.00", "stock": 5,
        }, headers={"Authorization": f"Bearer {self.seller_token}"}).json()["id"]

        resp = requests.post(f"{BASE_URL}/goods/skus/{sku_id}/restore_stock/", json={
            "quantity": 5
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        assert resp.status_code == 200
    ...
    def test_delete(self):
        """删除 SKU"""
        sku_id = requests.post(f"{BASE_URL}/goods/skus/", json={
            "spu": self.spu_id, "name": f"待删SKU_{int(time.time() * 1000)}",
            "price": "1.00", "stock": 1,
        }, headers={"Authorization": f"Bearer {self.seller_token}"}).json()["id"]

        resp = requests.delete(f"{BASE_URL}/goods/skus/{sku_id}/", headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
        assert resp.status_code == 204
...
class TestImageWrite:
    ...
    def test_upload_image(self):
        ...
        resp = requests.post(f"{BASE_URL}/goods/spus/{self.spu_id}/upload_image/", files={
            "image": (fake_img.name, fake_img, "image/png")
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        assert resp.status_code == 200
    ...
    def test_set_main(self):
        ...
        # 先上传一张图
        fake_img = self._fake_png()
        upload = requests.post(f"{BASE_URL}/goods/spus/{self.spu_id}/upload_image/", files={
            "image": (fake_img.name, fake_img, "image/png")
        }, headers={"Authorization": f"Bearer {self.seller_token}"})
        assert upload.status_code == 200
        img_id = upload.json()["data"]["id"]

        resp = requests.put(f"{BASE_URL}/goods/images/{img_id}/set_main/", headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
        assert resp.status_code == 200
    ...
    def test_delete_image(self):
        ...
        resp = requests.delete(f"{BASE_URL}/goods/images/{img_id}/", headers={
            "Authorization": f"Bearer {self.seller_token}"
        })
